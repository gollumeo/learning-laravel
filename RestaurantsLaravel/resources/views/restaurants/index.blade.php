...
<main class="w-4/5 md:w-2/3 grid grid-cols-2 self-center gap-6 text-base md:text-lg">
    @foreach($restaurants as $restaurant)
        <section
            class="border border-slate-400 self-center p-2 rounded-xl shadow-2xl hover:scale-105 transition h-full hover:bg-orange-900">
            <h2><strong>Name: </strong>{{ $restaurant->name }}</h2>
            <h3><strong>Location: </strong> {{ $restaurant->address }}
                , {{ $restaurant->zipCode }} {{ $restaurant->town }}, {{ $restaurant->country }}</h3>
